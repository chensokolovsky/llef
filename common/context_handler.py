...
class ContextHandler:
    ...
    def send_to_output(self, line):
        to_port = self.settings.send_to_local_port
        output_line(line, to_port)
        ...
        if self.frame.disassembly:
            ...
            for i, item in enumerate(instructions):
                if current_pc in item.split(':')[0]:
                    self.send_to_output(instructions[0])
                    if i > 3:
                        print_instruction(instructions[i - 3], TERM_COLORS[self.color_settings.instruction_color], should_send=to_port)
                        print_instruction(instructions[i - 2], TERM_COLORS[self.color_settings.instruction_color], should_send=to_port)
                        print_instruction(instructions[i - 1], TERM_COLORS[self.color_settings.instruction_color], should_send=to_port)
                        print_instruction(item, TERM_COLORS[self.color_settings.highlighted_instruction_color], should_send=to_port)
                        ...
                        for instruction in instructions[i + 1:i + 6]:  # noqa
                            # fmt: on
                            print_instruction(instruction, should_send=to_port)
                    if i == 3:
                        print_instruction(instructions[i - 2], TERM_COLORS[self.color_settings.instruction_color], should_send=to_port)
                        print_instruction(instructions[i - 1], TERM_COLORS[self.color_settings.instruction_color], should_send=to_port)
                        print_instruction(item, TERM_COLORS[self.color_settings.highlighted_instruction_color], should_send=to_port)
            ...
            self.send_to_output("No disassembly to print")
